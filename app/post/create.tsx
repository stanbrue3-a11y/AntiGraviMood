...
import { PlaceSkeleton } from '../../src/types/model';
import { IconService } from '../../src/services/IconService';
import { moodColors } from '../../src/design/tokens/colors';

const { width, height } = Dimensions.get('window');

// --- MOOD COLORS (Centralized from Design System) ---
const MOOD_COLORS = {
...
const getCategoryIcon = (place: PlaceSkeleton): keyof typeof Ionicons.glyphMap => {
  // Adapter le retour de IconService (string) vers keyof Ionicons
  const cat = IconService.getIconCategory(place);
  switch (cat) {
    case 'beer':
      return 'wine'; // Fallback mapping if needed, or update IconService to return ionic names
    case 'wine':
      return 'wine';
    case 'cocktail':
      return 'wine'; // 'wine' is generic for drink
    case 'cafe':
      return 'cafe';
    case 'restaurant':
      return 'restaurant';
    ...
    case 'palette':
      return 'easel'; // easel vs color-palette
    case 'leaf':
      return 'leaf';
    case 'museum':
      return 'library';
...
export default function CreateMomentScreen() {
  ...
  const [captionBgMode, setCaptionBgMode] = useState<'transparent' | 'black' | 'white'>(
    'transparent',
  );
        ...
        <View style={[styles.container, { backgroundColor: theme.background }]}>
          ...
          <View style={styles.searchWrapper}>
            <View
              style={[
                styles.searchBox,
                { backgroundColor: theme.surface, borderColor: MOOD_COLORS.chill },
              ]}
            >
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.placeInfo}>
                    <Text style={[styles.placeName, { color: theme.text.primary }]}>
                      {place.name}
                    </Text>
                    <View style={styles.placeMeta}>
                      <Text style={[styles.placeCategory, { color: moodColor }]}>
                        {place.category.toUpperCase()}
                      </Text>
                      {place.distance !== null && (
                        <Text style={[styles.placeDistance, { color: theme.text.secondary }]}>
    ...
    if (!permission?.granted) {
      return (
        <View style={[styles.container, styles.centered, { backgroundColor: '#000' }]}>
          ...
          >
            <LinearGradient
              colors={[MOOD_COLORS.chill, MOOD_COLORS.festif]}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
      ...
      <View style={{ flex: 1, backgroundColor: '#000' }}>
        ...
        >
          ...
          <View style={styles.sideControls}>
            ...
            <Pressable
              onPress={cycleTimer}
              style={[styles.sideBtn, timerSeconds > 0 && styles.sideBtnActive]}
            >
            ...
            <Pressable
              onPress={toggleGrid}
              style={[styles.sideBtn, showGrid && styles.sideBtnActive]}
            >
              ...
              <View style={styles.moodContainer}>
                  ...
                  {(['chill', 'festif', 'culturel'] as const).map((mood) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <Text style={[styles.moodText, { color: isSelected ? '#fff' : color }]}>
                          {label}
                        </Text>
                      </Pressable>
                    );
                ...
                >
                  <LinearGradient
                    colors={
                      selectedMood
                        ? [MOOD_COLORS[selectedMood], `${MOOD_COLORS[selectedMood]}cc`]
...
const styles = StyleSheet.create({
  ...
  sideBtnLabel: {
    ...
    color: '#fff',
    fontWeight: 'bold',
    backgroundColor: MOOD_COLORS.festif,
    paddingHorizontal: 4,
    paddingVertical: 2,
    borderRadius: 6,
    overflow: 'hidden',

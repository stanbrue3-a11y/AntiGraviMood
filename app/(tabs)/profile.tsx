...
export default function ProfileScreen() {
  ...
  const stats = useMemo(() => {
    const counts: Record<string, number> = { chill: 0, festif: 0, culturel: 0 };
  ...
  if (!isAuthenticated)
    return (
      <View style={[styles.guestContainer, { backgroundColor: COLORS.bg }]}>
        ...
        <Text style={{ color: COLORS.textPrimary, fontSize: 18, marginTop: 16, fontWeight: '600' }}>
          Profil Verrouillé
        </Text>
  ...
  return (
    ...
    >
      {/* 1. IDENTITY CARD */}
      <ProfileIdentity user={user} stats={stats} onLogout={handleLogout} />

      {/* 2. STATS SQUARES - CLICKABLE */}
      <ProfileStats friendsCount={stats.friendsCount} likedPlacesCount={likedPlaces.length} />

      {/* 3. MOODS (BARS KEPT) */}
      <View style={[styles.card, { backgroundColor: COLORS.card, borderColor: COLORS.cardBorder }]}>
        <Text
          style={[styles.serifTitle, { color: COLORS.textPrimary, marginBottom: 20, fontSize: 22 }]}
        >
          Mes moods
        </Text>
        <MoodRow
          label="Chill"
          color={moodColors.chill.primary}
          percent={stats.breakdown.chill}
          trackColor={COLORS.cardBorder}
          textColor={COLORS.textSecondary}
        />
        <MoodRow
          label="Festif"
          color={moodColors.festif.primary}
          percent={stats.breakdown.festif}
          trackColor={COLORS.cardBorder}
          textColor={COLORS.textSecondary}
        />
        <MoodRow
          label="Créatif"
          color={moodColors.culturel.primary}
          percent={stats.breakdown.culturel}
          trackColor={COLORS.cardBorder}
          textColor={COLORS.textSecondary}
        />
      </View>

...
async function main() {
  ...
  const candidatesForRepair = data.filter(
    (p) =>
      !p.is_verified || p.status === 'SCAFFOLDED' || !p.description || p.description.length < 200,
  );

  console.log(`\n🛠️ Candidates for repair (${candidatesForRepair.length} total):`);
  for (const p of candidatesForRepair.slice(0, 15)) {
    console.log(
      `- Slug: ${p.slug} | Name: ${p.name} | Rating: ${p.google_rating} | Status: ${p.status} | Verified: ${p.is_verified} | Desc length: ${p.description?.length || 0}`,
    );
  }
}

...
const payload = {
    ...
    tags: ['new_lot'],
    internal_audit_logs: ["[AUDIT F.I.P.P OK] Photos vérifiées : salle épurée beiges/nappes blanches, poisson gastronomique aux émulsions, mille-feuille fraise dressé minute."]
};

try {
    console.log("Updating Kigawa...");
    execFileSync('npx', ['tsx', 'scripts/agent_bridge.ts', '--update', 'kigawa', JSON.stringify(payload)], {
        stdio: 'inherit'
    });
} catch (e) {

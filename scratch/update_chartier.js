...
const payload = {
    ...
    tags: ['new_lot'],
    internal_audit_logs: ["[AUDIT F.I.P.P OK] Photos vérifiées : boiseries, verrière, escargots persillés et vin rouge."]
};

try {
    console.log("Updating Bouillon Chartier Montparnasse...");
    execFileSync('npx', ['tsx', 'scripts/agent_bridge.ts', '--update', 'bouillon-chartier', JSON.stringify(payload)], {
        stdio: 'inherit'
    });
} catch (e) {

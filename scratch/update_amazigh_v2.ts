...
const payload = {
    dominant_mood: 'chill',
    dogme_source: 'cuisine kabyle authentique, spécialité Laghmoudh (fèves et lait caillé), semoule aérienne, devanture bleue guinguette',
    ...
    insider_tip: "• Testez absolument le Laghmoudh, ce couscous kabyle sec aux fèves qui surprend par sa légèreté et son goût unique.\n• Aux beaux jours, demandez l'une des petites tables en terrasse sous les lampions pour profiter de l'ambiance de village du quartier.\n• Accompagnez votre repas d'un verre de Château Tellagh, un vin algérien sélectionné avec soin par le patron.",
    nearest_metro: 'Alésia',
    metro_lines: ['4'],
    has_terrace: true,
    reservation_policy: 'resa_conseillee',
    tags: ['new_lot', 'pastille_bleue'],
    internal_audit_logs: ["[AUDIT F.I.P.P OK] Photo 1 confirme le Laghmoudh et le Raïb. Validation sensorielle OK."]
};

try {
    execFileSync('npx', ['tsx', 'scripts/agent_bridge.ts', '--update', 'le-petit-amazigh', JSON.stringify(payload)], {

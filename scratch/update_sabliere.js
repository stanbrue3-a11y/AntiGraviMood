...
const payload = {
    ...
    internal_audit_logs: ["[AUDIT F.I.P.P OK] Photos vérifiées : store rouge et terrasse avec chaises rouges à l'extérieur (Index #0), galettes salées/sucrées excellentes et salle en bois/céramique à l'intérieur. Terrasse de trottoir validée."]
};

try {
    console.log("Updating Crêperie La Sablière...");
    execFileSync('npx', ['tsx', 'scripts/agent_bridge.ts', '--update', 'creperie-la-sabliere', JSON.stringify(payload)], {
        stdio: 'inherit'
    });
} catch (e) {

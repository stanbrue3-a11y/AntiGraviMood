...
const payloadTaverne = {
    ...
    internal_audit_logs: ["[AUDIT F.I.P.P OK] La photo montre une belle façade rouge et une terrasse avec des tables métalliques vertes."]
};
...
const payloadChow = {
    ...
    sensory_material: 'les tables en marbre froid et les néons rouges rappelant les ruelles hongkongaises',
    sensory_noise: 'le tintement des verres de vin et le ballet frénétique de la cuisine ouverte',
    description: "Derrière sa belle façade sombre, cette adresse nous plonge directement dans le Hong Kong des années soixante, avec une atmosphère feutrée et électrique aux airs de 'In the Mood for Love'. Autour des lourdes tables en marbre baignées par la lueur des néons rouges, la cuisine ouverte envoie une street-food frontale et moderne. On y partage de copieux paniers de dim sum fumants, des Dandan noodles et des toasts de crevettes frits, le tout arrosé d'un verre de vin naturel.",
    insider_tips: [
    ...
    tags: ['new_lot', 'pastille_rouge'],
    internal_audit_logs: ["[AUDIT F.I.P.P OK] Photo vérifiée : table en marbre, raviolis pimentés, poulet, vin rouge."]
};

try {
    console.log("Updating La Taverne de Zhao...");
    execFileSync('npx', ['tsx', 'scripts/agent_bridge.ts', '--update', 'la-taverne-de-zhao', JSON.stringify(payloadTaverne)], {
        stdio: 'inherit'
    });
    console.log("Updating MR CHOW...");
    execFileSync('npx', ['tsx', 'scripts/agent_bridge.ts', '--update', 'mr-chow-cuisine-chinoise', JSON.stringify(payloadChow)], {
        stdio: 'inherit'
    });
} catch (e) {

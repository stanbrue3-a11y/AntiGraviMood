...
const payload = {
    ...
    tags: ["pastille_rouge"],
    internal_audit_logs: ["[AUDIT F.I.P.P OK] Hero Immersion (Photo 0). Plats tradition (Profiterole Photo 1). Dogme Brasserie Trad validé. Status: TITAN."]
};

const output = execFileSync('npx', ['tsx', 'scripts/agent_bridge.ts', '--update', 'restaurant-14-juillet', JSON.stringify(payload)], { encoding: 'utf-8' });
console.log(output);

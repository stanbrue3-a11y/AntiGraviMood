            ...
            items: z.array(
              z
                .object({
                  ...
                  format: z
                    .enum(['assiette', 'verre', 'bouteille', 'planche'])
                    .optional()
                    .nullable(),
                })
                .passthrough(),
...
const RealTalkSchema = z
  .object({
    text: z.string().min(1).optional(),
  })
  .passthrough();

const PlaceEditorialSchema = z
  .object({
...
const rows = db
  .prepare(
    'SELECT id, name, pricing_json, real_talk_json, editorial_json, mood_scores_json FROM places',
  )
  .all();

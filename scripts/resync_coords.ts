...
async function resyncPlace(place: { slug: string; name: string; google_id: string; lat: number; lng: number }) {
  ...
  if (!loc) { console.log(`⚠️  SKIP  ${place.name} — géométrie Google absente`); return false; }
  ...
  if (maxDrift > DRIFT_THRESHOLD_METERS) {
    ...
    console.log(`🔧 CORRIGÉ  ${place.name} — dérive ${deltaLat.toFixed(0)}m lat / ${deltaLng.toFixed(0)}m lng`);
    return true;
  } else {
...
async function main() {
  ...
  let query = supabase.from('places').select('slug, name, google_id, lat, lng').not('google_id', 'is', null);
  ...
  if (slugIdx !== -1) {
    ...
    console.log(`📅 Mode : --recent ${recentDays}j (depuis ${since.substring(0, 10)}). Quota protégé.`);

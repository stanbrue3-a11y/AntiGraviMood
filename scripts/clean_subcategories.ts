...
async function main() {
  ...
  for (const place of places) {
    ...
    if (newTagsArray.length === 0 && oldTags.length > 0) {
      console.warn(
        `⚠️ ALERTE : ${place.name} (${place.slug}) n'a plus aucune catégorie après nettoyage ! Anciens tags: [${oldTags.join(', ')}]`,
      );
    }
  ...
  console.log(
    '👉 Pour appliquer les modifications, décommente la ligne de `update` dans le script.',
  );
}

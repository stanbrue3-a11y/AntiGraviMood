...
function extractMatch(content: string, regex: RegExp, defaultValue: string | null = null): string | null {
...
async function main() {
    ...
    for (const file of files) {
        ...
        try {
            ...
            let subcategories = [];
            if (subMatch) {
                try { subcategories = JSON.parse(subMatch[1].replace(/'/g, '"')); } catch(e){}
            }
            ...
            let metro_lines = [];
            if (metroMatch) {
                try { metro_lines = JSON.parse(metroMatch[1].replace(/'/g, '"')); } catch(e){}
            }
            ...
            const google_reviews_count = parseInt(extractMatch(content, /google_reviews_count:\s*(\d+)/, '0')!);
            ...
            const reservation_policy = extractMatch(content, /reservation_policy:\s*["']([^"']+)["']/, 'sans_resa');
            ...
            let google_photos = [];
            if (galleryMatch) {
                try { google_photos = JSON.parse(galleryMatch[1].replace(/'/g, '"')); } catch(e){}
            }

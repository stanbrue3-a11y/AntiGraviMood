...
const GPS_CENTERS: Record<number, { lat: number; lng: number; radius: number }> = {
  ...
  11: { lat: 48.8589, lng: 2.378, radius: 1300 },
  12: { lat: 48.8401, lng: 2.391, radius: 1800 },
  13: { lat: 48.8317, lng: 2.362, radius: 1800 },
  14: { lat: 48.8329, lng: 2.326, radius: 1500 },
  15: { lat: 48.8421, lng: 2.2986, radius: 2000 },
  16: { lat: 48.8638, lng: 2.275, radius: 2500 },
...
async function main() {
  ...
  if (placeIdFlagIndex !== -1 && args[placeIdFlagIndex + 1])
    forcePlaceId = args[placeIdFlagIndex + 1];
  ...
  try {
    ...
    if (businessStatus === 'CLOSED_PERMANENTLY' || businessStatus === 'CLOSED_TEMPORARILY') {
      console.error(
        `🛑 REJET STATUT : "${candidateName}" est signalé comme FERMÉ par Google Maps (${businessStatus}).`,
      );
      process.exit(1);
    }
    ...
    if (!isFood) {
      console.error(
        `🛑 REJET TYPE : "${candidateName}" n'est pas un établissement de restauration.`,
      );
    ...
    if (slug.length < 2) {
      console.error(
        `🛑 REJET SLUG : Slug généré "${slug}" trop court — vérifier le nom source : "${candidateName}"`,
      );
      process.exit(1);
    }
    ...
    const newPlace = {
      ...
      category: 'restaurant',
      subcategories: [],
      dominant_mood: moodResult.dominant_mood,
      address: details.formatted_address,
      arrondissement: extractedArr,

...
export const IconService = {
   ...
   * V2: Accepts both Place and PlaceSkeleton (only reads category + subcategories).
   */
  getIconCategory: (place: Place | PlaceSkeleton): IconCategory => {
    const cat = (place.category || '').toLowerCase();
    const subString = (
      Array.isArray(place.subcategories) ? place.subcategories.join(' ') : place.subcategories || ''
    ).toLowerCase();

    // 1. Bars & Drinks
    if (['bar', 'pub', 'biergarten', 'wine_bar', 'rooftop'].includes(cat)) {
      if (subString.includes('wine') || subString.includes('vin') || subString.includes('cave'))
        return 'wine';

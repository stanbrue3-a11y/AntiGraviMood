...
import { useTheme, palette, moodColors } from '../../design';
import { useSearchStore, SearchState } from '../../stores/searchStore';
...
const StealthSlider: React.FC<SlimSliderProps> = ({
  min,
  max,
  initialValue,
  onChange,
  ...
  useEffect(() => {
    const unsubscribe = useSearchStore.subscribe((state: SearchState) => {
  ...
  return (
    <View style={styles.sliderCol}>
      ...
      <View
        style={[styles.iconCircle, { borderColor: color + '40', backgroundColor: theme.surface }]}
      >
...
export const FloatingMapSliders = React.memo(() => {
  ...
  const activeSliders = useMemo(() => {
      ...
      return [
        {
          id: 'dish',
          label: 'Plat',
          icon: 'restaurant' as React.ComponentProps<typeof Ionicons>['name'],
          min: 10,
          max: 60,
          value: useSearchStore.getState().dishLimit ?? 20,
          onChange: setDishLimit,
          color: moodColors.culturel.primary, // Purple for Resto (aligns with culture/gastronomy)
          storeKey: 'dishLimit',
        },
      ];
    }
    ...
    if (
      selectedCategories.includes('bar') ||
      selectedCategories.includes('club') ||
      selectedCategories.includes('pub')
    ) {
      return [
        {
          id: 'pint',
          label: 'Conso',
          icon: 'pint-outline' as React.ComponentProps<typeof Ionicons>['name'],
          min: 3,
          max: 15,
          value: useSearchStore.getState().pintLimit ?? 7,
          onChange: setPintLimit,
          color: palette.warning, // Orange for Beer (aligns with festive/warm)
          storeKey: 'pintLimit',
        },
      ];
    }
      ...
      return [
        {
          id: 'coffee',
          label: 'Café',
          icon: 'cafe' as React.ComponentProps<typeof Ionicons>['name'],
          min: 1,
          max: 7,
          value: useSearchStore.getState().coffeeLimit ?? 3.5,
          onChange: setCoffeeLimit,
          color: moodColors.chill.primary, // Blue for Coffee (aligns with chill)
  ...
  // Dynamic layout calculations replacing magic numbers
  // Assuming Bottom Navbar is ~80px + safe area
  const dynamicFabPosBottom = insets.bottom + 80 + 20; // 20px padding above navbar
  const topLimit = insets.top + 210;
  const bottomLimit = SCREEN_HEIGHT - (dynamicFabPosBottom + FAB_SIZE + 45);

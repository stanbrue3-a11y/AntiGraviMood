...
export const DeepSearchScreen = ({
  searchQuery,
  results,
  onSelect,
  onClose,
}: DeepSearchScreenProps) => {
  ...
  const renderItem = ({ item, index }: { item: Place; index: number }) => {
    ...
    return (
      ...
      >
          ...
          <View style={styles.mainInfo}>
            ...
            <View style={styles.locationInfo}>
              <View style={styles.metroList}>
                {(item.location.metro_lines as (string | number)[])?.map(
                  (line: string | number, i: number) => (
        ...
        {item.description && (
          <Text
            style={[styles.description, { color: isDark ? '#D1D5DB' : '#4B5563' }]}
            numberOfLines={2}
          >
            {item.description}
          </Text>

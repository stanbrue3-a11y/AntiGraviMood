...
export function AnimatedTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  return (
    <BlurView intensity={60} tint="dark" style={styles.absoluteContainer}>
...
const TabItem = memo(({ isFocused, icon, label, onPress, onLongPress, isDark }: TabItemProps) => {
  // Generate a random color only when focused changes to true
  const randomColor = useMemo(
    () => MOOD_PALETTE[Math.floor(Math.random() * MOOD_PALETTE.length)],
    [isFocused],
  );
  ...
  return (
    <Pressable onPress={onPress} onLongPress={onLongPress} style={styles.tabItem} hitSlop={10}>

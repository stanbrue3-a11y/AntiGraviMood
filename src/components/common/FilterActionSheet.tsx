  ...
  const MoodCard = ({ mood, label, emoji }: { mood: MoodType; label: string; emoji: string }) => {
    ...
    return (
      ...
      >
        ...
        <Text
          style={[styles.moodLabel, { color: isActive ? theme.background : theme.text.primary }]}
        >
          {label}
        </Text>
      </Pressable>
      ...
      <View style={styles.overlay}>
        ...
        >
          ...
          >
            ...
            {/* ZONE 1.5: BUDGET (Restored) */}
            <Text style={[styles.sectionTitle, { color: theme.text.secondary, marginTop: 24 }]}>
              BUDGET MAX
            </Text>
            <View style={{ paddingHorizontal: 4 }}>
              <MagneticPriceSelector
                value={maxPrice === null ? 3 : [5, 10, 20, 50].indexOf(maxPrice)}
                onChange={(idx) => {
                  const prices = [5, 10, 20, 50];
                  setMaxPrice(prices[idx]);
                }}
              />
            </View>
            ...
            <View
              style={[styles.powerUpContainer, { backgroundColor: theme.surfaceElevated + '40' }]}
            >
              <PowerUp
                label="Happy Hour en cours"
                icon="beer-outline"
                value={filterHappyHour}
                onChange={setFilterHappyHour}
              />
              <PowerUp
                label="Terrasse Ensoleillée"
                icon="sunny-outline"
                value={filterTerrace}
                onChange={setFilterTerrace}
              />
              <PowerUp
                label="Ouvert Maintenant"
                icon="time-outline"
                value={filterOpenNow}
                onChange={setFilterOpenNow}
              />
            </View>

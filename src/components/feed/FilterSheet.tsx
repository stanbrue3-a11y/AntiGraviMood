...
import { useSearchStore, PLACE_CATEGORIES } from '../../stores/searchStore';
...
const CATEGORY_STYLE: Record<
  string,
  { primary: string; secondary?: string; icon: keyof typeof Ionicons.glyphMap }
> = {
        ...
        <Animated.View
          style={[
            ...
            isSelected
              ...
              : [styles.catItemInactive, { backgroundColor: theme.surface, borderColor: border }],
            animatedStyle,
          ]}
        >
          {backgroundElement}
...
const RollingCounter = ({ theme }: { theme: any }) => {
  return (
    <Animated.Text style={[styles.applyText, { color: theme.background }]}>Explorer</Animated.Text>
  );
};
...
export const FilterSheet = ({ visible, onClose }: FilterSheetProps) => {
  ...
  const {
    ...
    setSelectedDistrictsGlobal,
    timeRangeGlobal,
    setTimeRangeGlobal,
    filterOpenNowGlobal,
    setFilterOpenNowGlobal,
    selectedMoodsGlobal,
    setSelectedMoodsGlobal,
  ...
  const [localTime, setLocalTime] = useState<{ start: number; end: number }>({
    start: 18,
    end: 26,
  });
  ...
  useEffect(() => {
    if (visible) {
      setLocalMoods(
        selectedMoodsGlobal.length > 0 ? selectedMoodsGlobal : ['chill', 'festif', 'culturel'],
      );
  ...
  const handleDistrictToggle = useCallback((d: number) => {
    setLocalDistricts((prev) => (prev.includes(d) ? prev.filter((x) => x !== d) : [...prev, d]));
  }, []);
  ...
  // Local results count calculation removed in V3 (SQLite source of truth)
    ...
    >
      <View style={[styles.container, { backgroundColor: theme.background }]}>
        ...
        >
          ...
          <View style={styles.section}>
            <View style={styles.headerWithAccent}>
              ...
              <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>Vibe</Text>
            </View>
          ...
          >
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                ...
                <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>Créneau</Text>
              </View>
          ...
          <View style={styles.section}>
            <View>
              <View style={styles.headerWithAccent}>
                ...
                <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>Quartier</Text>
              </View>
              <Text
                style={[styles.sectionSubtitle, { color: theme.text.secondary, marginBottom: 12 }]}
              >
                {localDistricts.length > 0
        ...
        <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 20) }]}>
          <TouchableOpacity
            style={[styles.applyButton, { backgroundColor: theme.text.primary }]}
            onPress={handleApply}
            activeOpacity={0.9}
          >
            <RollingCounter theme={theme} />
          </TouchableOpacity>
        </View>
      </View>
    </Modal>

...
export const DiscoverHeader = ({
  ...
  const { selectedCategories, toggleCategory, searchQuery, setSearchQuery } = useSearchStore(
    useShallow((state) => ({
  ...
  const { setMapCameraRequest } = useUIStore();
  const placesMap = usePlacesStore((state) => state.placesMap);
  const filteredIds = useSearchStore((state) => state.filteredIds);

  const filteredPlaces = React.useMemo(
    () => filteredIds.map((id) => placesMap[id]).filter((p): p is Place => !!p),
    ...
    >
      ...
      {isSearchActive ? (
        <View
          style={[
            styles.header,
            { paddingHorizontal: 20, flexDirection: 'column', alignItems: 'stretch' },
          ]}
        >
          <View
            style={[
          ...
          >
            <Ionicons
              name="search"
              size={20}
              color={isDark ? '#8ccaf7' : theme.text.secondary}
        ...
        <View style={styles.header}>
          <ScalePressable
            ...
            style={[styles.roundBtn, { backgroundColor: '#23252E' }]}
          >
            {transparent ? (

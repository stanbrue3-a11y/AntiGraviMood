...
export const getPriceReference = (category: string): CategoryPriceReference => {
  ...
  if (
    normalized.includes('resto') ||
    normalized.includes('bistro') ||
    normalized.includes('bouillon')
  ) {
 ...
 */
export const calculatePinceScore = (placePrice: number, categoryAverage: number): number => {
